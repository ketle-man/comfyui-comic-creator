...
- **3D姿势** — 在分格内放置 VRM/GLB/GLTF 模型并摆姿势后烘焙到图像中，支持可拖动的视线目标追踪、摇动骨骼（头发/裙子）物理效果，以及带可拖动风向标记的微风效果。"放置到分格"按钮左侧的滑块可调整相机的**FOV（视场角，10～120°）・Near（近裁剪面，0.01～5）**（通过 [comfyui-vrm-pose-editor](#可选依赖) 联动）
