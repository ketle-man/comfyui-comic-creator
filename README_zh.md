...
- **模板功能** — 通过导入 SVG 或使用向导（画线分割页面）创建并注册分格模板。SVG 导入不限制绘图软件：Inkscape、Illustrator、Affinity Designer、CorelDraw 等主流工具的输出格式（`rect`/`path`/`polygon`/`polyline`/`circle`/`ellipse`，包含嵌套分组与 transform）均可直接读取，只需可视化地画出分格线并保存即可。将模板加入模板分组后，会在排版标签的素材面板「模板」标签中以可折叠的分组标题显示，模板数量增多时也能更容易地找到目标模板
