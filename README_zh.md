...
- **绘画工具** — 与矢量图形绘制完全独立的光栅画笔功能。点击"添加绘画"会按所选分格/覆盖层/草稿层的尺寸创建透明图像，自动选中并开启绘制ON，可立即用画笔绘制。支持画笔颜色、不透明度、大小（滑块），以及x5（5倍画笔大小）和橡皮擦的ON/OFF切换按钮（通过颜色明确显示状态）。绘画对象在图层面板中以🖌图标和"绘画"名称显示，与普通图像区分开。可用于Image标签中绘制・编辑的草图，或制作I2I参考图像
...
- **I2I联动** — 将选中图像（I2I）或当前整页导出的PNG（PI2I）发送到 Workflow Studio 的 Generate UI，并可将生成结果送回。可在设置标签中配置自动加载的默认工作流（通过 [ComfyUI-Workflow-Studio](#可选依赖) 联动）
