...
- **半自动漫画制作** — 通过"将此页流式填入版面"按钮，将排版标签中已应用模板的页面各分格与脚本的分格按分格编号顺序对应。"自动生成对话气泡"按钮可基于该对应关系，在各分格中自动生成填入台词的对话气泡（固定为圆角矩形，之后可用现有手动编辑功能调整）。"L"按钮与"批量生成图像"按钮提供基于Workflow Studio的提示词起草（Ollama/LM Studio）与按分格宽高比批量生成图像
