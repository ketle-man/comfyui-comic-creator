...
一款运行在 ComfyUI 之上的漫画页面制作单页应用（SPA）。以"作品"（页面组）为单位管理页面，可从模板创建页面，在分格中放置图像、对话气泡、文字、图形和 3D 姿势，并可导出为 JPEG/PNG/WebP/SVG/PDF/EPUB 格式。内置基于图层的图像编辑器、字体管理、AI 图像生成（Nanobanana）以及脚本管理，力求用单个节点完成漫画创作的整个流程。
...
- **模板功能** — 通过导入 SVG 或使用向导（画线分割页面）创建并注册分格模板。SVG 导入不限制绘图软件：Inkscape、Illustrator、Affinity Designer、CorelDraw 等主流工具的输出格式（`rect`/`path`/`polygon`/`polyline`/`circle`/`ellipse`，包含嵌套分组与 transform）均可直接读取，只需可视化地画出分格线并保存即可。会根据 SVG 的 `width` 属性（带单位的实际尺寸）自动判断是否需要缩放校正以使分格边框宽度与其他模板保持一致，如需要会弹出确认对话框供你查看并调整倍率（校正为内部坐标缩放"1用户单位=0.01mm"）。将模板加入模板分组后，会在排版标签的素材面板「模板」标签中以可折叠的分组标题显示，模板数量增多时也能更容易地找到目标模板。开启向导中的"设计"复选框后，可创建页面外及分格间都不留空白/边框的**设计模板**（适用于传单制作等用途）——边框宽度会被强制设为0，将模板放置到页面后，分格边界线的粗细仍可通过排版标签的"分格边框宽度"独立调整。页面标签"模板"子标签中、分组筛选下拉菜单右侧的**"作为页面插入"**按钮，同样可以将选中的模板作为新页面插入到当前打开的作品中（与从排版标签素材面板"模板"插入的处理相同）
- **输出** — 导出为 JPEG/PNG/WebP/SVG/PDF/EPUB 格式，支持多页面批量导出与自动编号文件名。相关库（jsPDF/JSZip）已随应用打包，所有格式在离线环境中也可导出
- **SVG输出（在外部软件中重新编辑）** — 不经栅格化直接以矢量形式导出，文字保留为真正的 `<text>` 元素，对话气泡形状保持为矢量路径，所用字体以Base64自包含嵌入。经实测确认，在Inkscape、Affinity Designer/Publisher中几乎可以直接重新编辑。Adobe Illustrator会导致图像链接失效（需逐一重新指定），不适合实际的重新编辑；CorelDraw读取时会卡死，不支持
...
### 排版标签
- **图像放置** — 拖放图像到分格或叠加层中（拖放到叠加层：可先在图层面板选中"叠加层"再拖放，或直接拖放到叠加层中已有的内容上）。通过手柄调整大小（默认锁定纵横比，按住 Alt 拖动可解除锁定自由变形）和旋转。工具栏中的**删除**按钮不仅限于图像，还可删除当前选中的任意对象（对话气泡、文字、图形、分组），效果与Delete/Backspace键相同
...
- **对话气泡** — 在分格内放置椭圆、圆角矩形、思考、爆炸、云朵（蓬松/波浪）等形状的对话气泡，支持8点调整手柄。**无尾巴**复选框可将尾巴长度设为0并隐藏（取消勾选会恢复为之前的长度）。爆炸形状带有**内侧弧度**滑块，可在保持尖刺尖端锐利的同时让凹角和侧边变圆润。**内嵌文字**按钮可为以上任意形状内嵌自动换行的文字（支持竖排、行距滑块、文字颜色、粗体、独立于文字对齐的上下/左右位置调整、Google/系统/分类字体选择，双击已内嵌文字的气泡可重新编辑）。素材中的自定义SVG气泡放置后也可修改填充色・边框色
...
- **文字** — 支持横排/竖排，Google Fonts / 系统字体，可设置填充、描边、勾边字、阴影、行距、上下对齐、文字对齐的样式弹窗。预览文字栏支持多行，"大小"下方的行距滑块，以及与对话气泡内嵌文字相同的按钮式上下对齐（上/中央/下）・文字对齐（左/中央/右）可用于调整确认（竖排时上下对齐＝列内纵向位置，文字对齐＝列群左右位置）。行距・上下对齐・文字对齐会实际应用于排版标签和Image标签两侧的文字。填充除纯色外还支持渐变、纹理（可指定X/Y位置）和无填充（排版/Image标签通用）
...
- **绘画工具** — 与矢量图形绘制完全独立的光栅画笔功能。点击"添加绘画"会按所选分格/覆盖层/草稿层的尺寸创建图像，自动选中并开启绘制ON，可立即用画笔绘制（开启左侧的"背景色"复选框可改为创建指定颜色填充的不透明图像，避免发送到I2I时背景变黑）。画笔不仅可用于新添加的绘画对象，也可直接在当前选中的任意普通图像上绘制。支持画笔颜色、不透明度、大小（滑块），以及x5（5倍画笔大小）和橡皮擦的ON/OFF切换按钮（通过颜色明确显示状态）。**使用数位板时，画笔大小和不透明度会根据笔压自动变化**（鼠标操作时与以往一样保持固定）。绘画对象在图层面板中以🖌图标和"绘画"名称显示，与普通图像区分开。"统合选中图像"按钮可将图层面板中勾选的2个以上图像/绘画对象合成为一张PNG（保留叠放顺序和旋转），便于将多个图层合并后发送到I2I。可用于Image标签中绘制・编辑的草图，或制作I2I参考图像
- **蒙版工具** — 可为分格、覆盖层或单个对象（图像、对话气泡、文字、图形、分组）叠加多个蒙版图层并合成，分为"隐藏"和"显示"两种类型。涂抹时可调整画笔（涂抹/擦除）、大小、硬度，并可将隐藏区域以半透明红色预览。支持反转/清除/全部涂抹的批量操作，以及按图层临时启用/禁用。**使用数位板时，画笔大小会根据笔压自动变化**（鼠标操作时与以往一样保持固定）
- **3D姿势** — 在分格内放置 VRM/GLB/GLTF 模型并摆姿势后烘焙到图像中，支持可拖动的视线目标追踪、摇动骨骼（头发/裙子）物理效果，以及带可拖动风向标记的微风效果。工具栏的滑块可调整相机的**FOV（视场角，10～120°）・Near（近裁剪面，0.01～5）**。**🎛 Editor** 按钮用于打开"Light & Pose Editor"（始终从Pose标签开始，可在顶部切换💡Light/🕺pose标签）：Pose标签包含形态键、📚 Pose Library（从`poses/`文件夹的缩略图中选择.json/.vroidpose/.vrma姿势并立即应用；.vrma文件可在迷你播放器中预览，并通过"🔑 Load KEY"导入到关键帧时间线）以及关键帧时间线（切换Pose/Camera/Light/Wind轨道，用播放条拖动到任意帧后点击"📸 Capture"即可将该帧的姿势直接确定到分格）；Light标签用于管理多个灯光（Sun/Point/Spot/Box/Ambient）、Ground/BG Wall以及预设的保存（通过 [comfyui-vrm-pose-editor](#可选依赖) 联动）
- **3D文字** — 使用Three.js将包含日文在内的文字、或SVG文件的矢量路径立体挤出后烘焙到图像中（"文字""SVG"模式切换），支持TrueType Collection（.ttc，游黑体・Meiryo等Windows标准CJK字体常见格式）。材质可从标准（金属度・粗糙度）或卡通（兼容MToon的着色）中选择，并可**分别指定表面色与侧面色**（对几何体的正反面盖面与挤出侧面分别应用不同材质）。**⚙ 3D设置（3D Text Editor）**弹窗采用三栏布局：左侧为灯光／材质／相机标签切换面板，中央为直接嵌入弹窗的实时预览，右侧为常驻显示的文字面板（内容・字体・对齐・行距）。倒角的厚度・大小・分段数也可细致调整，若相对于形状过大会自动禁用并显示警告。缩放操作模式与增强抗锯齿与3D姿势的灯光编辑器共用设置（通过 [comfyui-vrm-pose-editor](#可选依赖) 联动）
...
- **PixiJS FX** — 在"图像"子标签中为选中图像应用粒子・滤镜效果。支持多个滤镜叠加与排序，组合可保存/读取/删除为预设（通过 [comfyUI-particle-pixijs](#可选依赖) 联动）
...
- **Select / Text / 3D Text / Draw / Shape / Fill / Mask / Blur / Filter / BG Remove / Upscale** 各工具（Select 工具支持通过扩展操作浮层显示并操作超出画布边界的对象的选择框和手柄，并在透明区域绘制真实棋盘格背景以明确画布边界；3D Text 与排版标签共用同一引擎和设置弹窗，通过 [comfyui-vrm-pose-editor](#可选依赖) 联动）
- **Select工具的裁剪／扩展画布** — <b>裁剪</b>可通过可拖动的裁剪范围浮层（8个手柄）或在属性面板中直接输入X/Y/宽/高数值指定裁剪范围后执行，会调整整个画布尺寸，并保持各图层内容不变、仅平移位置（可撤销）。其右侧的<b>扩展画布</b>按钮可在上/右/下/左四个方向分别以像素为单位添加边距（勾选"联动四边"可一次性统一设置四边），边距颜色可选择透明・白色・黑色（选择白色或黑色时会自动在最背面添加填充图层以反映到导出图像中）。两者均在属性面板中显示数值输入与执行/取消按钮
...
- **创建草稿画布** — "草稿"按钮（New 右侧）无需输入尺寸对话框，直接创建与当前作品相同宽高比、72dpi换算尺寸的画布（用于粗略草图）。"排版"按钮会以整页尺寸插入到排版标签的草稿图层
- **PSD (Photoshop) 支持** — "Open PSD"按钮可连同图层结构一起读取PSD文件（各图层的不透明度・混合模式・显示/隐藏状态也会一并还原，不会与现有内容合并，始终作为新文档打开）。"PSD"保存按钮可在保持图层结构的同时导出为PSD文件（由于PSD无法将调整图层表示为独立图层，导出前会将其效果烘焙到紧邻其下方的可见图层中）
- **操作栏：Save / Send to** — 整理为 Save 系（PNG / PSD / Project / Gallery）与 Send to 系（Eagle / wsI2I / LI Node / 排版）。**wsI2I** 是原来的"发送到I2I"（发送到 Workflow Studio 的 Generate UI Image 输入插槽）；**LI Node** 是新功能，将合成结果直接发送到 ComfyUI 画布上当前选中节点（如 Load Image）的 image 控件（仅在通过 ComfyUI 的"CC"按钮打开时可用，与 [ComfyUI-Workflow-Studio](#可选依赖) 的"Send to Workflow"使用相同机制）
- **Draw工具的滴管** — 通过取色器旁的按钮直接从画布拾取颜色作为笔刷颜色。**使用数位板时，画笔大小和不透明度会根据笔压自动变化**（鼠标操作时与以往一样保持固定）
- **Shape工具的Same Layer模式** — 每次绘制图形时叠加到同一图层，而不是新建图层。矩形・椭圆的填充除纯色外还支持渐变和纹理（可指定X/Y位置）
- **Fill工具** — 单色填充，或线性/放射状渐变填充（颜色渐变条与方向面板）
- **Mask工具** — Paint/Color/Alpha/Text/Vector/Shape各子工具，也支持SAM3分割和ABR笔刷（工具构成参考了 [comfyui-mask-editor-one](#致谢) 实现）。Paint子工具的画笔大小**会根据数位板笔压自动变化**（鼠标操作时与以往一样保持固定）。安装 Workflow Studio 后子工具栏会新增 **Inpaint** 按钮，可通过遮罩+提示词进行生成式AI局部重绘（通过 [ComfyUI-Workflow-Studio](#可选依赖) 联动）
- **Outpaint（Workflow Studio联动）** — 通过Mask工具中Inpaint右侧的**Outpaint**按钮使用（无需提前创建蒙版图层）。设置上/右/下/左边距（可联动指定）、边距填充方式（扩展像素／透明／白色／黑色）、Positive/Negative提示词、Grow Mask By、Denoise后点击Run，即可一次性完成：①合成按边距扩展后的底图 → ②自动生成蒙版（白色=待生成的边距、黑色=需保留的原图）→ ③发送至Workflow Studio执行生成，成功后画布会实际扩大并将结果作为新图层添加。由于扩散模型的VAE编码/解码往返在图像尺寸不是64px倍数时会切除若干像素（导致结果出现透明缝隙），发送前会自动将实际扩展的边距调整为64px的倍数。默认工作流可在设置标签的"Outpaint设置"中独立于Inpaint设置指定
- **PixiJS FX** — 通过工具栏按钮为当前图层应用粒子・滤镜效果。支持多个滤镜叠加与排序，组合可保存/读取/删除为预设（通过 [comfyUI-particle-pixijs](#可选依赖) 联动）
...
- 创建并保存汇总填充・描边・勾边字・阴影・行距・上下对齐・文字对齐（与对话气泡内嵌文字相同的按钮形式，实际应用于排版/Image标签两侧的文字）的"样式"，以及字体+尺寸+样式的"预设"，可从排版/Image标签立即调用
...
- **媒体类型（漫画/半自动漫画/小说/剧本）** — 可按作品固定切换编辑界面的子标签基础架构。"漫画"与"半自动漫画"共用同一个分格编辑界面，但各作品数据相互独立（小说、剧本计划在后续版本中支持）
- **半自动漫画制作** — 通过"将此页流式填入版面"按钮，将排版标签中已应用模板的页面各分格与脚本的分格按分格编号顺序对应。"从排版获取"按钮可将本页的分格数与当前选中的排版标签页面的实际分格数对齐（若会删除已有数据的分格会显示确认对话框）。分格表中每行台词都有"对话气泡形状"下拉框（默认/普通/圆角矩形/思考/炸弹/云蓬松/云波浪），"自动生成对话气泡"按钮可基于该对应关系，在各分格中按各行指定的形状（默认为圆角矩形）自动生成填入台词的对话气泡（之后可用现有手动编辑功能调整）。"L"按钮提供基于Workflow Studio的提示词起草（Ollama/LM Studio）。"批量生成图像（T2I）"按钮会打开与"批量生成图像（I2I）"相同结构的专用弹窗（整体Positive/Negative与各分格提示词合并、可选择是否跳过提示词为空的分格、可指定T2I默认工作流），按分格宽高比批量生成图像（T2I）。"批量生成图像（I2I）"按钮会打开与排版标签I2I弹窗等效的专用弹窗（整体Positive/Negative与各分格提示词合并、可选择是否跳过提示词为空的分格），以各分格当前的图像为输入进行批量I2I生成。"批量生成图像（Nanobanana）"按钮改用Nanobanana（Gemini图像生成API）而非Workflow Studio，自动选择与各分格边界框长宽比最接近的Nanobanana分辨率预设并在插入时缩放（附带模型选择与"2K"开关，生成图像遵循Eagle自动保存设置）。Nanobanana标签也新增了相同的"2K"开关（对应Gemini API的ImageConfig.imageSize，仅部分模型支持）。由于Gemini图像生成API没有用于控制I2I变化程度的数值参数，原有的I2I强度滑块已从Nanobanana标签及半自动漫画弹窗中移除
...
基础功能不需要任何额外的 Python 依赖包（`aiohttp` / `Pillow` 已随 ComfyUI 本体自带）。仅 Image 标签的 PSD (Photoshop) 支持使用了 `psd-tools`，已记录在 `requirements.txt` 中，通过 ComfyUI Manager 等方式安装时会自动引入（未安装也不影响其他功能）。
...
<p>
  <img src="docs/11_pixifx.png" width="260" alt="PixiJS FX 多滤镜叠加与排序">
...
<p>
  ...
  <img src="docs/16_3dtext.png" width="260" alt="3D文字">
</p>
<p>
  <img src="docs/17_3DPose_1.png" width="260" alt="3D姿势工具栏（🎛 Editor 按钮）">
  <img src="docs/17_3DPose_2.png" width="260" alt="Light & Pose Editor（Pose标签、关键帧时间线）">
</p>
