...
- **半自动漫画制作（部分已实现）** — 通过"将此页流式填入版面"按钮，将排版标签中已应用模板的页面各分格与脚本的分格按分格编号顺序对应。"自动生成对话气泡"按钮可基于该对应关系，在各分格中自动生成填入台词的对话气泡（固定为圆角矩形，之后可用现有手动编辑功能调整）。通过Workflow Studio联动按分格批量生成图像计划在后续更新中支持
