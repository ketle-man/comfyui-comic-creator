...
- **半自动漫画制作** — 通过"将此页流式填入版面"按钮，将排版标签中已应用模板的页面各分格与脚本的分格按分格编号顺序对应。"自动生成对话气泡"按钮可基于该对应关系，在各分格中自动生成填入台词的对话气泡（固定为圆角矩形，之后可用现有手动编辑功能调整）。"L"按钮与"批量生成图像"按钮提供基于Workflow Studio的提示词起草（Ollama/LM Studio）与按分格宽高比批量生成图像（T2I）。"批量生成图像（I2I）"按钮会打开与排版标签I2I弹窗等效的专用弹窗（整体Positive/Negative与各分格提示词合并、可选择是否跳过提示词为空的分格），以各分格当前的图像为输入进行批量I2I生成
