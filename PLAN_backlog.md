...
- **半自動マンガ作成 Phase 3（Workflow Studio連携バッチ画像生成）**（Phase 1・2は2026-08-03実装完了・詳細はDEVLOG参照）: 参考実装`ComfyUI_sloppy-comic`を踏まえた3フェーズ計画の最終フェーズ。各コマの画像プロンプト（手動入力＋コマ割り表の「L」ボタンによるLLM補助）から、コマのbboxアスペクト比（`08-panels-images.js`の`getBoundingBoxFromPoints`で取得可能）に応じたサイズでWorkflow Studio経由の画像を順次生成し、対象コマの`state.selectedPanelId`を切り替えてから`insertImage`で挿入する（sloppy-comicの逐次forループと同じ設計）。Workflow Studio側（別リポジトリ`ComfyUI-Workflow-Studio`）の`gallery-tab.js`に新規ブリッジ`_wfmReceiveLLMPromptRequest`（`ai-tab.js`の`callLLM`/`callChat`を呼ぶ、既存の`_wfmReceiveImageForI2I`等と同じ配置パターン）の追加が必要。画像生成自体は既存の`window._wfmGenerateTab.generate(workflow)`（Workflow Studio側で公開済み、新規実装不要）をコマごとに順次呼ぶだけでよい。コマのbbox比率→解像度のマッピングはsloppy-comicの`sdxl_ratio_to_res`相当を踏襲。将来的にスクリプトタブ内または別タブへAIチャット機能を追加する構想があるため、このブリッジは単発プロンプト下書き用途に限定した最小実装とし、用途別に関数を独立させておく（既存のI2I/Inpaint/Select I2Iブリッジ関数と同じ構成）。
