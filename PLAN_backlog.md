...
更新日: 2026-08-01（アセットパネル「A」「F」タブ復帰漏れ不具合の修正、フキダシ「テキストを内包」に上下/左右の独立配置調整を追加。詳細はDEVLOG参照）
...
- **半自動マンガ作成 Phase 2・3**（Phase 1は2026-08-03実装完了・詳細はDEVLOG参照）: 参考実装`ComfyUI_sloppy-comic`（台本内`{タグ}`埋め込みでコマ単位プロンプト/セリフを切り出し、Masonry風レイアウトへ自動合成するComfyUIカスタムノード）を踏まえた3フェーズ計画の残り2フェーズ。
  - **Phase 2（フキダシ自動配置）**: Phase 1の「このページをレイアウトに流し込む」による対応付け結果を使い、各コマにフキダシを自動生成してセリフを流し込む。`09a-balloon-init.js`/`09b-balloon-shapes.js`にプログラム的に呼び出せる新規エクスポート関数（`createBalloonProgrammatic(panelId, {type, cx, cy, rx, ry, tail...})`等）、`09f-bubble-text.js`にテキスト設定用エクスポート（`setBalloonText(el, text)`等）を追加する必要がある。配置ロジックはv1では「コマ内で上から均等配置」の単純方式（sloppy-comicのエッジ密度ヒューリスティックのような高度な自動配置は見送り、自動生成後は既存の手動編集機能で再編集可能なため精緻さは不要というユーザー判断）。スクリプト側でフキダシ形状（角丸/思考/バクダン/雲等）を指定する機能は今回は見送り（将来アイデアとして保留）。
  - **Phase 3（Workflow Studio連携バッチ画像生成）**: 各コマの画像プロンプト（手動入力＋コマ割り表の「L」ボタンによるLLM補助）から、コマのbboxアスペクト比（`08-panels-images.js`の`getBoundingBoxFromPoints`で取得可能）に応じたサイズでWorkflow Studio経由の画像を順次生成し、対象コマの`state.selectedPanelId`を切り替えてから`insertImage`で挿入する（sloppy-comicの逐次forループと同じ設計）。Workflow Studio側（別リポジトリ`ComfyUI-Workflow-Studio`）の`gallery-tab.js`に新規ブリッジ`_wfmReceiveLLMPromptRequest`（`ai-tab.js`の`callLLM`/`callChat`を呼ぶ、既存の`_wfmReceiveImageForI2I`等と同じ配置パターン）の追加が必要。画像生成自体は既存の`window._wfmGenerateTab.generate(workflow)`（Workflow Studio側で公開済み、新規実装不要）をコマごとに順次呼ぶだけでよい。コマのbbox比率→解像度のマッピングはsloppy-comicの`sdxl_ratio_to_res`相当を踏襲。将来的にスクリプトタブ内または別タブへAIチャット機能を追加する構想があるため、このブリッジは単発プロンプト下書き用途に限定した最小実装とし、用途別に関数を独立させておく（既存のI2I/Inpaint/Select I2Iブリッジ関数と同じ構成）。
