...
更新日: 2026-09-26（v1.47.0リリース: Autoタブお題入力のモーダル化・画像生成タブ分離・レイアウトタブへのコマ分割ツール追加、オートレイアウトサブタブのヘルプ英語・中国語訳追加。詳細はDEVLOG参照）
...
- **Autoタブ: WFSの翻訳・VLMツールの取り込み**（2026-09-21発生・未着手）: 「ストーリー・脚本」サブタブ実装時に見送った項目。Workflow Studioの`ai-tab.js`が持つ翻訳・VLM（画像解析）ツールをAutoタブのChatに登録式ツールとして追加するかどうか、レイアウトサブタブの作業が一区切りついた現時点で改めて要否をユーザーと相談すること。
- **Autoタブ: I2I（画像を入力にした生成）への対応**（2026-09-21発生・未着手）: 初版は方針としてT2Iのみとし見送った。2026-09-25にレイアウトタブのGenerateモーダルへ「Autoから取得」機能（ストーリー・脚本テキストをGenerateモーダルのプロンプト欄へ転記）を追加したが、これはAuto自身のI2I機能ではなく既存レイアウトGenerateモーダル経由の代替。Autoタブのチャット/画像生成UIから直接I2Iを行いたい場合は別途着手が必要。
- **オートレイアウトのコマ直接編集（移動・拡大縮小・回転・削除）の代替案検討**（2026-09-25発生・未着手）: コマ間余白の自動計算（辺単位オフセット）や脚本パネルとの1対1対応という既存の前提と、特に回転機能が根本的に相性が悪いため直接編集の実装自体は見送り済み（`NOTES_auto-layout-panel-edit.md`参照）。代わりに「レイアウト転送後、既存レイアウトタブ側で制約（脚本対応・隙間なしレイアウトの維持等）をON/OFFできる編集機能」という方向性をユーザーが希望。着手時は転送済みページと元のAuto脚本ページの対応関係が正式には保存されていない点（`page_auto_<timestamp>_<index>`という命名規則に手がかりがある程度）を踏まえて計画すること。

---

## 完了済み（記録）

- **Autoタブ（AIマンガ自動作成）の「ストーリー・脚本」サブタブを追加**（2026-09-21 完了・未リリース、ブランチ`feature/auto-tab`）: 参考プロトタイプ`manga_panel`を参考に、お題からストーリー→脚本を2段階でAI生成し、Chatで相談・編集、Chat内から画像生成（ローカル/Gemini、output/cc_auto保存）できる新タブ。ローカルLLM（Ollama/LM Studio/Lemonade/Unsloth）とGeminiに対応（LLMクライアントはWFSから移植、設定はCC側に保存）。既存スクリプトタブとは連携しない新規作品。以前の「コマ割り自動生成は行わない」方針は撤回済み。未対応: 「レイアウト」サブタブ（脚本からのコマ割り自動生成・重要度按分・既存レイアウトタブ連携）、WFSの翻訳/VLMツール、I2I。計画は`PLAN_auto_tab.md`、詳細はDEVLOG 2026-09-21。ヘルプ・README 3言語更新済み。
