...
更新日: 2026-09-09（v1.40.0: Imageタブのキャンバス外操作性向上・3Dテキスト確定時解像度改善・Fillグラデーション操作ペンタブ対応。詳細はDEVLOG参照）
...
- **「Auto」タブの追加（AIによるマンガ自動作成、第1弾＝お題からの全ページ脚本生成）**（2026-09-21計画作成、実装は未着手・ユーザーが後日開始）: 参考アプリ`now_work/manga_panel`の機能のうちCCに無い「お題＋総ページ数→脚本のLLM生成」を新規タブで実装する。AIエンジンはWorkflow Studio AI TOOLの設定（Ollama/LM Studio等）とNanobananaタブのGemini APIキーを利用。以前の「コマ割り自動生成は行わない」方針は撤回済み（自動作成→人が編集するフロー）。2026-09-21追記: ユーザー仕様確定（Autoタブは「ストーリー・脚本」「レイアウト」の2サブタブ、ストーリー・脚本を先に完成→レイアウトへ。3ペイン=左お題/中央ストーリー・脚本・メモ/右Chat・設定。AI設定はCC側保存でWFS AI TOOL設定と同形、GeminiはNanobananaのキー流用。既存スクリプトタブとは連携せず新規作品。TOOLで画像生成対応）。同日、確認点4件の回答を反映（LLMクライアントはWFSからCCへ移植、作成は2段階、Chatでストーリー・脚本を編集＋Auto向けツール登録式、翻訳/VLMはレイアウト後、画像生成はローカル/Geminiラジオ選択・保存先`output/cc_auto/`）。残りの確認事項もすべて確定（Chatからの反映=反映ボタン方式、脚本タブ=構造化表、画像生成=生成ボタン主・T2Iのみ初版、ストーリー空なら脚本生成無効、履歴は作品に保存等）。計画は確定済みで、ユーザーの指示があればそのまま実装に着手できる。詳細は`PLAN_auto_tab.md`と`docs/plan/auto_tab_mockup.png`参照。
