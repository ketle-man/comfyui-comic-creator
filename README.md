...
- **半自動マンガ作成（一部実装済み）** — 「このページをレイアウトに流し込む」ボタンで、レイアウトタブのテンプレート適用済みページとスクリプトのコマをパネル番号順に対応付け。「フキダシを自動生成」ボタンで、対応付け結果をもとにセリフ入りフキダシ（角丸矩形固定）をコマ内に自動生成（生成後は既存の手動編集機能で調整可能）。Workflow Studio連携によるコマ単位バッチ画像生成は今後のアップデートで対応予定
