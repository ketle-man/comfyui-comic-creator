...
- **出力** — JPEG/PNG/WebP/PDF/EPUB 形式で出力。複数ページの一括出力・連番ファイル名にも対応

### レイアウトタブ

- **画像配置** — ドラッグ＆ドロップでコマに画像を挿入。ハンドルでリサイズ・回転
- **フキダシ** — 楕円・角丸矩形・思考・バクダン等の形状をコマ内に配置。8 点リサイズハンドル対応
- **テキスト** — 縦書き・横書き、Google Fonts / システムフォント、塗り・線・袋文字・影を設定できるスタイルモーダル
- **形状描画（ドロー）** — 矩形・楕円・直線・曲線・多角形・鎖・ロープ・My 曲線を SVG レイヤーに直接描画
- **3D ポーズ** — VRM/GLB/GLTF モデルをコマ内に配置してポーズを付け画像に焼き込み（[comfyui-vrm-pose-editor](#依存関係任意) 連携）
...
- **Shape ツールの Same Layer モード** — 図形を描くたびに新規レイヤーを作らず、同じレイヤーに重ね描き
...
<!-- 公開後、以下のプレースホルダーを実際の GitHub URL に置き換えてください -->

ComfyUI Manager の「Install via Git URL」から以下の URL を入力してインストールできます（公開後に有効化予定）：
...
- **comfyui-vrm-pose-editor** — 3D ポーズ編集機能を提供するコンパニオンノード
- **ComfyUI-Workflow-Studio** — I2I 連携・ギャラリー埋め込み機能を提供するコンパニオンノード
- **comfyUI-particle-pixijs** — PixiJS FX（パーティクル・フィルタ効果）機能を提供するコンパニオンノード
- **comfyui-mask-editor-one** — Image タブの Mask ツール・レイヤー機構の実装時に参考にしたノード
