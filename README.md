...
- **テンプレート機能** — SVG インポートまたはウィザード（線を引いてコマを分割）でコマ割りテンプレートを作成・登録。SVG インポートは Inkscape・Illustrator・Affinity Designer・CorelDraw など主要な描画ソフトの出力形式（`rect`/`path`/`polygon`/`polyline`/`circle`/`ellipse`、入れ子グループ・transform 含む）にツールを問わず対応しており、コマの区切り線を視覚的に描いて保存するだけで読み込める。テンプレートグループを作成しておくと、レイアウトタブのアセットパネル「テンプレート」タブで折りたたみ可能なグループ表示になり、テンプレート数が増えても目的のものを探しやすい
