...
- **3D ポーズ** — VRM/GLB/GLTF モデルをコマ内に配置してポーズを付け画像に焼き込み。視線ターゲット追従・揺れボーン物理・そよ風エフェクト（発生源マーカーで向き指定）にも対応。「コマに配置」ボタン左隣のスライダーでカメラの**FOV（画角、10〜120°）・Near（ニアクリップ面、0.01〜5）**を調整可能（[comfyui-vrm-pose-editor](#依存関係任意) 連携）
