...
## 2026-09-26（Autoタブお題入力のモーダル化・画像生成タブ分離・レイアウトタブへのコマ分割ツール追加、v1.47.0）

Autoタブの右ペイン改善3件と、レイアウトタブへの新機能追加1件を実施した。いずれもユーザー自身が実機で確認済み。

**Autoタブ: お題・プロンプト入力のモーダル化、2ペイン化**: 「ストーリー・脚本」サブタブの左ペイン（お題・プロンプト入力、ページ数、作成/サンプルボタン）を、既存の`.text-dialog-overlay`パターンを流用したモーダル（`#auto-theme-dialog`）へ移設し、中央+Chatの2ペイン構成に変更した（`auto-2pane`、Chat幅を380px→480pxに拡大）。作業バーに開くボタン（`#auto-theme-open-btn`）を新設し、ストーリー作成・サンプル読込成功時は自動でモーダルを閉じて中央ペインへ切り替える。ステータス表示はモーダル専用の`auto-theme-status`と、作業バーの保存フィードバック用`auto-status`に分離した（`28-auto-tab.js`）。

**Autoタブ: 脚本コマからChatの画像生成プロンプトへの送信**: 脚本タブの各コマの「＋ セリフ」ボタン右隣に**「Send Chat Image」**ボタンを追加し、そのコマの背景描写・演技指示をワンクリックで画像生成プロンプトへ送信（既存入力は上書き）できるようにした（`onScriptClick`の新規`send-chat-image`分岐）。

**Autoタブ: 画像生成をChat右隣の独立タブに分離**: 従来Chat内の`<details>`折りたたみパネルだった画像生成UIを、`Chat`/`画像生成`/`設定`の3タブ構成に再編し、Chatの右隣の独立タブへ分離した（`#auto-right-image`新設、旧`.auto-img-panel`のdetails/summary用CSSは削除）。`switchRightTab`の対象に`'image'`を追加し、「Chatの返答を画像プロンプトへ」ボタン・上記のSend Chat Imageボタンのどちらも、この新しい画像生成タブへ自動的に切り替わるようにした。

**レイアウトタブ: コマ分割ツールの新規追加**: 「分割」ツールをツールバーに新設し、プレビュー上でコマ内をドラッグした任意角度の直線に沿って、そのコマ1つだけを2つに分割できるようにした（`24a-panel-split.js`新設）。幾何計算はテンプレート作成ウィザードの単一コマ分割と同じ`_splitPolygonByLine`（`06a-polygon-geometry.js`）を再利用しつつ、既存コンテンツ（画像・フキダシ・テキスト）を持つ実コマの分割に対応するため、分割後の内容振り分け（bounding boxが新コマ側に完全に収まる要素だけ新コマへ移動、分割線をまたぐ・旧コマ側の要素はそのまま残す）を新規実装した。旧コマはid・番号を変えずpointsだけ縮小し、新コマは旧コマの直後に挿入する（新コマ番号=旧+1、以降繰り下げ）。ロック中のコマは分割不可、サブコマは対象外。

実装当初、分割後の2コマの間に枠線が引かれない不具合が見つかった。原因はコマの見た目の枠線がコンテンツ層ではなく背景SVG（`svgContent`）に静的に描かれた`polygon`（id=コマID）が担っていたためで、コンテンツ層のクリップ・画像等は正しく分割していたが、この背景ポリゴンだけ元の形のまま放置されていた。分割時にこのポリゴンも複製・分割するよう修正した。

あわせて「コマ間の幅」入力を追加し、分割後の2コマの間に他のコマ同士と同じ見た目の隙間を確保できるようにした。レイアウトタブは常にコマ枠線幅の半分ずつを内側へ自動で食い込ませて表示するため（`renderLayoutTab`のクリップ縮小）、`_splitPolygonByLine`へ渡す実際のポリゴン間隔は「入力値－現在のコマ枠線幅」に補正している（そのまま渡すと枠線幅の分だけ余計に広く見えてしまうため）。さらに、分割ツールをONにした時点・「分割」サブタブを開いた時点で、そのページの既存コマ同士の隙間（矩形の軸方向の隙間の中央値）を自動検出して入力欄へ反映するようにし、手入力の手間を無くした。最後に使った値はlocalStorageに保存する。

**実機検証（Playwright、ComfyUI_5）**: Autoタブの一連の変更、およびレイアウトタブのコマ分割（境界をまたぐ吹き出しが旧コマに残るケース／完全に新コマへ収まるケースの両方、枠線の生成、コマ間の幅の自動検出と実際の見た目の一致、リロード後の永続化）を実機で確認した。開発リポジトリの変更は都度ComfyUI_5の`custom_nodes\comfyui-comic-creator`側へ手動同期している（シンボリックリンクではないため）。

**ヘルプ・README反映**: 上記すべてについて、ヘルプタブ（`22-help-tab.js`）とREADME（3言語）を更新した。日本語（`_HELP_DATA`）・英語・中国語（`_HELP_I18N`）すべてに新規見出し「コマ分割」を追加し、Autoタブの該当見出しも更新した。作業中、英語版の追加文に未エスケープのアポストロフィ（"once it's done"）があり構文エラーを起こしていたのを発見・修正した（`node --check`では検出できず、過去にも同種の事故があったため`vm.SourceTextModule`で検証した）。

**追記（同日）: オートレイアウトサブタブの英語・中国語訳を追加**: 上記のヘルプ反映時点では見送っていた、Autoタブ「オートレイアウト」サブタブの8見出し（オートレイアウトサブタブ — 概要／ページ設定／テンプレート／詳細設定／中央ペイン: プレビューとツール／右ペイン: プロパティ／レイアウトタブへの転送／Generateモーダルとの連携）について、ユーザー依頼を受けて英語・中国語訳を追加した（以前のセッションから未反映のまま残っていた既存debt）。あわせて、Overview/概述見出しに残っていた「"Layout" will be added later」という古い説明（オートレイアウト自体はv1.45.0で実装済み）を、日本語版と同様に2つのサブタブを説明する内容へ修正し、Basic Workflow/基本流程の末尾にオートレイアウトサブタブへ進む手順を1項目追加した（日本語版はモーダル化対応時に追記済みだったが英語・中国語版は未追従だった）。`node --check`に加え`vm.SourceTextModule`による深層構文検証も実施し、ComfyUI_5への同期・`diff --strip-trailing-cr`での差分なし確認も行った。PLAN_backlog.mdの該当debtエントリは完了として除去した。バージョン更新・リリースは未実施。

---

## 2026-09-25（Autoタブ Chat Toolの多言語対応、レイヤーパネルへの個別アイテムのアセット登録機能、アセットパネル「I」タブのレイアウトタブ対応、v1.46.0）

前回セッション（v1.45.0）に続けて同日中に、ユーザー報告・依頼を受けて3件の機能追加を実施した。いずれもユーザー自身が実機で確認済み（本セッションでのKapture等による実機検証は行っていない）。

**Autoタブ「ストーリー・脚本」Chat Toolの多言語対応**: ストーリー・脚本の本文生成は既にUI設定言語（日本語/English/中文）に対応していたが、Chat機能のツール（`AUTO_TOOLS`: 脚本化・セリフ推敲・ストーリー整理・キャラクター設定）がチャット欄へ送る依頼文と、Chatのシステムプロンプトが「簡潔な日本語で答えてください」で固定されており、UI言語をen/zhにしてもツール実行時の発言・AIの返答が日本語のままになる不具合があった。`auto-story-core.js`に`TOOL_PROMPT_BY_LANG`（ツールごとの依頼文をja/en/zhで用意）と`getToolPrompt(toolId, lang)`を新設し、`AUTO_TOOLS`からは`prompt`フィールドを削除。`buildChatSystemMessage()`に`lang`引数を追加し、既存の`langLabel(lang)`（ストーリー・脚本生成で使っていたもの）を使って回答言語の指示を動的化した。呼び出し元（`28-auto-tab.js`の`onChatToolRun()`・`sendChat()`）で`getLang()`を渡すよう変更。

**レイヤーパネル: 個別アイテムのアセット登録**: 「アセット登録を１つのオブジェクトで登録可能にしたい」という要望を受け、レイヤーパネル上部（↑↓ボタンの左隣）に**📦+**ボタンを新設した。従来、アセット登録はグループ行の📦ボタン（グループ化済みオブジェクトが対象）のみだったため、チェックボックスで選択した未グループ化の個別アイテム（画像・フキダシ・テキスト・描画図形）を対象に追加した。実装は`05-groups-move.js`の`saveGroupAsAsset()`が内包していたダイアログ処理（登録先グループ選択・ファイル名入力・保存API呼び出し）を`_openSaveAssetDialog(defaultName, buildPayload, onSaved)`として切り出し、新設した`saveSelectedLayersAsAsset()`と共有する構成にした。ユーザーからのフィードバックを受け、実装後に3点の仕様調整を行った: ①複数チェック時は「複数アイテムを1つにまとめて登録」ではなく1件のみ受け付け、2件以上で押すとメッセージを表示するよう変更（`layer.saveAssetOnlyOneItem`）。②登録完了後、対象アイテムのチェックボックスを自動的に解除するよう`onSaved`コールバックを追加。③チェックした1件が画像の場合、従来は（グループアセットと同様に）`<g data-group-id>`でラップしたSVGとして保存していたため、コマへ挿入時に画像1枚だけなのに余計なグループが生成される問題があった。これを避けるため、`py/ccc.py`の`handle_save_group_asset`に`svg`と排他の`image`フィールド（画像のdata URL）を追加し、指定時はSVGラップせずMIMEタイプ（png/jpeg/webp/svg+xml）に応じた拡張子でそのままバイナリ保存するようにした（他の画像アップロード系エンドポイントに合わせ`_read_json_limited`によるサイズ制限も追加）。フキダシ・テキスト・描画図形は引き続きSVGグループとして保存し、挿入時に編集可能なネイティブ要素として復元されるようにしている。

**アセットパネル「I」タブのレイアウトタブ対応**: 「レイアウトのアセットパネルIタブでimageフォルダも表示可能にしたい（ImageタブのアセットパネルIタブと同じ）」という要望を受け、`01-state.js`の`switchTab()`内、「I」タブボタンの表示条件を`tabId === 'image'`のみから`tabId === 'image' || tabId === 'layout'`に拡張した。「I」タブの中身（`#image-asset-tree`）自体はメインタブに関わらず常に描画済みだったため、ボタンの表示切替だけで対応できた。挿入ボタン（`image-asset-insert-btn`、Aタブの「コマに挿入」・Iタブの「Open」はどちらも同じ`handleInsertAsset()`を共有）は元々「アクティブなメインタブがimageかどうか」で分岐していたため、レイアウトタブでは自動的に「選択中のコマへ画像として挿入」の経路（既存のA タブと同じ経路）に乗り、追加のロジック実装は不要だった。

**ヘルプ反映**: 上記3件について、ヘルプタブ（`22-help-tab.js`）の「はじめに」画面構成・「レイアウト」レイヤーパネル/グループ機能・「アセットパネル」概要/Imageタブ(I)→画像プロジェクトタブ(I)に改名・「Image」プロジェクトの保存と再編集の各見出しを日英中3言語とも更新した。見出し数がJA/EN/ZHで一致することをNode.jsスクリプトで機械検証済み（過去に未エスケープのアポストロフィで構文エラーを起こした事故があったため`node --check`に加え`vm.SourceTextModule`でも構文検証した）。README（3言語）にも同内容を反映済み。

**未対応**: なし（本セッションの内容はヘルプ・README・バージョン更新・リリースまで完了）。

---

## 2026-09-25（フキダシ表示/非表示の誤保存バグ修正・オートレイアウト×テンプレート連携・レイヤーパネルAlt+クリック一括切替、v1.45.0）

前回セッション（Generate連携等）に続けて同日中に、ユーザー報告のバグ修正1件と、新機能2件を実施した。

**フキダシ表示/非表示の誤保存バグ修正**: 「レイアウトタブでフキダシを非表示にしても、別タブへ移動して戻ると表示に戻る（非表示のまま保存すれば維持される）」という報告を受け調査した。原因は`04b-layer-panel-render.js`の`makeShapeItem`（フキダシ）・`makeTextItem`（テキスト）のvis-btn/lock-btn/delete-btnハンドラが、保存先パネルIDを`state.selectedPanelId`（キャンバス上で最後に選択していたコマ）から求めていたこと。レイヤーパネルはページ内の全コマのオブジェクトを1つの一覧にまとめて表示するため、操作対象のオブジェクトと選択中パネルが食い違うケースがあり、その場合は無関係な別コマへ誤って保存され、対象コマの変更はDBに反映されないまま（タブ復帰時の再構築で古い表示に戻る）という不具合だった。画像アイテム（`makeImageItem`）は既に「DOM上の実際の親コマ（`closest('g[data-clip-panel]')`）を優先する」修正が入っていたため、同じパターンをフキダシ・テキスト・描画図形（`makeDrawShapeItem`）にも展開した。Kaptureで、選択中パネルと異なるパネルのフキダシを非表示にしてタブ復帰しても維持されることを確認済み。

**オートレイアウト × ページタブのテンプレート連携**: 「オートレイアウトで作ったコマ割りをテンプレートとして保存したい」「テンプレート数が増えるとオートレイアウト側の狭い左ペインに一覧が収まらない」という要望・懸念を受け、テンプレートの一覧閲覧・管理をページタブ側に一本化する設計に変更した。ページタブのテンプレート一覧（`06c-template-wizard.js`）から各テンプレートカードの個別ボタンを廃止し、代わりに上部・「ページとして挿入」の右隣に**「オートレイアウトへ送信」**ボタンを新設（選択中の1件に対して動作、常に1件のみ送信可能・送信し直すと置き換わる、`06b-template-manager.js`の`_tmplAutoStage`でlocalStorage管理）。オートレイアウトの「テンプレート」タブ（`28b-auto-layout-tab.js`）は一覧表示をやめ、送信された1件だけを表示するよう書き換えた。あわせて、オートレイアウトに現在表示中のページ（脚本のこのページのコマ割り。まだフキダシ・画像は実体化されていない段階）をページタブの新規テンプレートとして保存できる**「テンプレートとして保存」**ボタンを新設した（`auto-layout-core.js`の`quadsToPageLayout()`で変換）。「選択解除」ボタンは、そのページのテンプレート指定を解除するのに加えて送信状態自体もあわせて解除するようにし（送信を取り消す専用ボタンは持たない方針）、ボタン名も「テンプレート指定を解除（自動生成に戻す）」→「選択解除」、「表示中のページをテンプレートとして保存」→「テンプレートとして保存」とそれぞれ短縮した（下に説明文があるため簡略化、ボタン見切れの改善も兼ねる）。実装前にレイヤーパネルの各オブジェクト種別（フキダシ/画像/テキスト/描画図形/グループ/マスクレイヤー）で表示/非表示の仕組みが同じか調査し、ユーザーに確認を取ってから進めた。Kaptureで、送信・解除・オートレイアウト側への反映・ページ→テンプレート保存（非対称スタイルのコマ割りが正しく反映されること）を確認済み（検証用に作成したテンプレート・ページは削除済み）。

**レイヤーパネルのAlt+クリック一括表示/非表示切替**: レイアウトタブのレイヤーパネルで、表示/非表示アイコンをAlt+クリックすると、クリックしたオブジェクトと**同じ種類**（フキダシ／画像／テキスト／描画図形のいずれか）のオブジェクトを、**ページ全体**（全コマ・オーバーレイ・下書きレイヤー、グループ内の子要素も含む）にわたって一括で表示/非表示切替する機能を追加した（`04b-layer-panel-render.js`の`_rlpBulkToggleSameType`）。対象が複数のコマ/オーバーレイ/下書きにまたがる場合は、実際に変更が生じた範囲だけそれぞれ個別に保存する。グループ自体（表示/非表示ボタンが元々存在しない）とマスクレイヤー（表示/非表示の仕組みが別物）は対象外とした。テキストの収集では、フキダシに内蔵されたセリフ用`<text>`（バクダン等の内包テキスト）を誤って含めないよう、`.balloon-shape`の子孫を除外するフィルタを入れている。Kaptureの`evaluate`で実際の`<g class="balloon-shape">`を2つの異なるコマへ注入し、Alt+クリック相当の`altKey:true`付き合成クリックイベントをvis-btnへ発火させて、両方が一括で非表示になりIndexedDBの両コマそれぞれに個別保存されることを確認した（Kaptureの`click`ツール自体は装飾キー付きクリックに対応していないため、この検証手法を採った。検証用データは削除済み）。

**オートレイアウト詳細設定の既定フォントサイズ変更**: `blankLayoutSettings()`（`28-auto-tab.js`）の既定`fontSizePt`を28→80に変更（レイアウトタブの既定サイズに合わせる、ユーザー指定）。新規作成するAuto作品にのみ適用され、既存の保存済み作品の値は変わらない（ユーザーの現在の作品はそのままでよいとの回答済み）。

**ヘルプ反映**: レイアウトタブ「レイヤーパネル」・ページタブ「テンプレート」の使い方・Autoタブ「テンプレート（左ペイン）」の説明を日英中3言語とも更新した。作業中、英語版の追加文に未エスケープのアポストロフィ（`Auto Layout's`）があり構文エラーを起こしていたのを発見・修正した（`node --check`では検出できず、過去にも同種の事故があったため`vm.SourceTextModule`で検証した）。

**インストール先の同期漏れに関する訂正**: 前回セッションまでのメモリに「`comfyui-comic-creator`はシンボリックリンクで自動反映される」とあったが誤りと判明した（`Get-Item`の`LinkType`が空でリパースポイントではないことを実機確認）。以後、変更ファイルは`custom_nodes\comfyui-comic-creator\`側へ手動コピーが必要（`comfyui-workflow-studio`と同様）。

**未対応**: README（3言語）・PLAN_auto_tab.mdは本セッションで反映済み。バージョン更新・リリースは未実施。

---

## 2026-09-25（Autoタブの多言語対応バグ修正・LLM出力の頑健パース強化・レイアウトGenerate連携、v1.45.0）

前日の作成言語対応（UI設定言語に応じてストーリー・脚本の本文をLLMへ言語指示する機能）に対し、ユーザーから2件の指摘があり修正した。続けて、ストーリー・脚本の作成時にLLM出力が読み取れなかった不具合の調査・修正と、レイアウトタブのGenerateモーダルとAutoタブを連携させる機能追加も行った。

**見出しの言語混在バグ**: 英語UIでストーリーを作成すると、見出しが「# 【Title】」のように日本語の【】記号と英語が混在して出力されることが判明した。原因は`storySystemPrompt(lang)`（`auto-story-core.js`）が見出し（【タイトル】【登場人物】【あらすじ】【ページ配分】）を日本語固定のまま渡し、本文だけ他言語で書くよう指示していたこと。モデルが見出し記号自体まで律儀に（あるいは中途半端に）訳そうとして崩れていたと考えられる。見出し記号自体もUI設定言語に応じて切り替える`STORY_HEADINGS`（ja: 【タイトル】等／en: `[Title]`等／zh: 【标题】等）を新設し、システムプロンプトに「この記号のまま使う（翻訳しない）」旨を明記した。ストーリー本文は他の場所でパースされていないため、見出し記号を変えても実害はない。

**サンプルが日本語固定だった**: 「サンプル」ボタン（LLM未接続でも流れを試せる機能）で読み込まれるお題・ストーリー・脚本（`SAMPLE_THEME`/`SAMPLE_STORY`/`SAMPLE_SCRIPT`）が日本語固定だったため、UI設定言語を切り替えても常に日本語の内容が読み込まれていた。同じ内容（柴犬の宇宙飛行士ポチが月面で宇宙骨を見つける話）を英語・中国語にも翻訳し、`SAMPLE_THEME_BY_LANG`/`SAMPLE_STORY_BY_LANG`/`SAMPLE_SCRIPT_BY_LANG`として言語別に保持。`getSampleTheme(lang)`/`getSampleStory(lang)`/`cloneSampleScript(lang)`にAPIを変更し、呼び出し元（`28-auto-tab.js`の`onSample()`）で`getLang()`の値を渡すようにした。

**実機検証（Kapture、多言語対応バグ修正分）**: UI設定言語を英語→中国語→日本語の順に切り替えて「サンプル」ボタンを押し、それぞれお題・見出し・本文がすべて対応言語で一貫して表示されること（英語UIでは`[Title]`等、中国語UIでは【标题】等）を確認した。日本語（既定）でのサンプル・システムプロンプトの出力が変更前と完全一致することはNode.jsで検証済み。

**脚本パースの頑健化（LLM出力の閉じ括弧不足を自動修復）**: ユーザーが「ストーリーから脚本を作成」時に「LLMの出力から脚本を読み取れませんでした」というエラーに遭遇し、提示された実際の生出力を調査した。原因はLLMの出力がpage2のパネル配列の途中で打ち切られており（`pages`配列とルートオブジェクトの閉じ括弧`]}`が2つ欠落。おそらくMax tokens到達）、JSONとして不完全だったこと。末尾に`]}`を1つ補うだけで完全な有効JSONに復元できることを確認した。さらに、既存の「行ベース復元」フォールバックは改行区切りを前提にしていたため、改行のない1行JSON出力（今回のケース）では機能せず、結果的に読み取り失敗になっていたことも判明。`repairTruncatedJson()`（`auto-story-core.js`）を新設し、文字列リテラルの中身を無視しつつ開き括弧をスタックで数え、不足分の閉じ括弧（と閉じられていない文字列があればその閉じクォート）を末尾に補ってから再度JSON.parseを試みるステップを、通常のJSON.parse失敗後・行ベース復元の前に`parseScriptResponse()`へ追加した（新しい`method: 'json-repaired'`）。ユーザー提示の実際の壊れたJSONで全2ページ分すべてが正しく復元されることをNode.jsで検証済み（既存の正常系・行ベース復元・完全に読み取れないケースへの影響もないことを確認）。

**レイアウトタブ「Generate」モーダルとAutoの連携（Autoから取得）**: レイアウトタブのGenerateモーダル（`openLayoutI2IModal()`、`15-pixifx-bridge.js`）に「Autoから取得」機能を追加した。現在Autoタブで作業中の作品の脚本ページをセレクトで選び、ボタンを押すと、その作品のストーリー全文を「全体」タブへ、選んだ脚本ページの各コマの背景描写・演技指示（下描きガイド）を対応する「コマN」タブへ、それぞれPositiveプロンプトとして反映する（既存の入力は上書き）。コマの対応付けは配列の順序のみで行い、レイアウトタブへの転送機能（詳細編集）とは完全に独立させた（転送済みページと元のAuto脚本ページの対応関係は現状正式には保存されておらず、`page_auto_<timestamp>_<index>`というIDの命名規則に手がかりがある程度のため、それには依存しない設計にした）。これによりI2Iバッチ生成にもAutoで作ったストーリー・脚本のデータを活用できるようになった。

**実機検証（Kapture、脚本パース・Generate連携分）**: レイアウトタブのGenerateモーダルで「Autoから取得」を押し、「全体」タブへストーリー全文が、「コマ1」タブへ対応するactionが正しく反映されることを確認した。

**未対応**: README（3言語）・ヘルプは本件すべて反映済み。バージョン更新・リリースはv1.45.0として実施済み（訂正: 本行は当時「未実施」と記載していたが、後日確認したところ次セッションのv1.45.0リリースに含まれていた）。

---

## 2026-09-24（オートレイアウトサブタブの改善8件・ヘルプ反映、v1.45.0）

前日までに実装した第2弾「オートレイアウト」サブタブを、ユーザーが実際に使う中で挙がった改善要望に対応した。詳細は`PLAN_auto_tab.md`§12-8。ブランチ`feature/auto-tab`のまま。

**フォントタブ連携（既定フォントに設定ボタン）**: フォントタブ右パネルの「選択テキストに適用」ボタンの下に「オートレイアウト」欄を新設し、「既定フォントに設定」ボタンでオートレイアウトの詳細設定タブと同じ`work.layout.fontFamily`を直接設定できるようにした（確認ダイアログなしで即座に自動保存）。`28-auto-tab.js`の`getAutoWork()`を、`auto.work`が未初期化（Autoタブを一度も開いていない）状態でも呼べるようnullセーフ化した。フォント名への拡張子表示も要望に上がったが、実現にはブラウザのLocal Font Access APIでバイナリを読み込みヘッダー判定する追加処理が必要と分かったため、ユーザー判断で見送った。

**コマ選択プロパティのUI調整**: 右ペインのフキダシ「変更」「追加」ボタンを2行目へ移動（セリフ選択セレクトを1行目に単独化）。画像プロンプト入力欄を既定7行表示・`resize:vertical`でドラッグによるさらなる拡張に対応させた。

**転送時のz-orderバグ修正**: レイアウトタブへの転送処理（`insertBalloonsAndImages()`）が「フキダシ作成→画像挿入」の順で処理していたため、`insertImage()`が常にコマのg要素内の最後（＝最前面）に`<image>`を追加する仕様と噛み合わず、画像がフキダシを覆い隠すことがあった。処理順序を「画像挿入→フキダシ作成」に入れ替え、画像挿入後（DOM再構築後）に最新のパネルレイヤーSVGを取得し直してからフキダシを追加するよう修正した。フキダシは常に画像より前面になる（フキダシ同士の重なりは従来どおり許容）。

**コマ間余白と外側余白の独立化（2段階で修正）**: `auto-layout-core.js`の`quadToPagePoints()`（コマ間余白の実現方法）を2段階で修正した。第1段階は、従来「全頂点を四角形の重心方向へ一律にinset」していたためページ外周（＝外側余白の境界）に接する頂点も内側へ縮み、外側余白とコマ間余白の半分が合算されて見えていた問題を、頂点の座標成分が外周と一致する場合はその成分を固定するよう修正。しかしこれには副作用があり、非対称・斜め継ぎ目スタイルで共有辺を挟む両側のコマの重心位置が異なると、区切り線が斜めに歪んで見えることがユーザー実機で発覚した。第2段階として、**辺単位の多角形オフセット**（各辺をその内向き法線方向へgapPx/2だけ平行移動し、隣接辺同士の交点を新頂点にする）方式に書き換え、共有辺は常に元の辺と平行になるよう修正した（Node.jsで斜め共有辺でも法線方向の距離が正確にgapPxになることを数値検証済み）。

**生成のランダム性**: 「生成（全ページ）」ボタンを押すたびに新しい疑似乱数シード基点（`alo.seedBase`）を採番し、非対称・斜め継ぎ目スタイルの配置が毎回変化するようにした。シード自体は作品データには保存せず、ページ送り等での再生成は同一セッション内のシードを使い回して一貫性を保つ。

**ツールボタンのUI改善**: 選択/分割ツールのアクティブ状態のスタイルを、ボーダー強調のみから他の同種トグルボタン群と共通の青背景に変更した。「分割」ツール1つを「分割(縦)」「分割(横)」の2ボタンに分離し、コマ分割の軸（縦線で左右に分けるか、横線で上下に分けるか）をユーザーが明示的に選べるようにした（従来はコマの幅・高さ比較による自動判定のみ）。

**作成言語のUI設定言語対応**: 「作成」（ストーリー生成）・「ストーリーから脚本を作成」時、CCのUI設定言語（日本語/English/中文）に応じてLLMへ言語指示を追加するようシステムプロンプトを書き換えた。脚本のJSON構造・キー名・`importance`/`bubbleType`の値は言語に関わらず維持し、`"action"`・`"character"`・`"text"`の中身のみ設定言語で書くよう指示する。実際にその言語で書かれるかは使用モデルの能力次第（結果は保証しない）。日本語（既定）の出力は変更前と完全一致することを確認済み。

**ヘルプ反映**: ヘルプタブのAutoタブヘルプで「レイアウトは今後追加予定」のまま放置されていた記述を更新し、オートレイアウトサブタブ全体（ページ設定・テンプレート・詳細設定・中央ペイン・右ペイン・レイアウトタブへの転送）の説明を新規追加した。フォントタブヘルプにも「オートレイアウト連携（既定フォントに設定ボタン）」を追加した。

**実機検証（Kapture）**: 既定フォント設定ボタンの自動保存とオートレイアウト詳細設定への反映、フキダシ変更/追加ボタンの2行目配置、画像プロンプト欄の7行表示・リサイズ、転送後のフキダシが画像より前面に表示されること、外側余白0・コマ間余白1500pxでも外周がページ端にぴったり接すること、非対称・斜め継ぎ目スタイルで区切り線が平行になること、「生成」ボタン連打で毎回配置が変わること、分割(縦)・分割(横)ツールそれぞれの軸での手動分割、ツールボタンの青背景表示、ヘルプタブの表示、をいずれも確認。

**未対応**: README（3言語）は当時未反映だったが、後続セッションでv1.45.0リリース前に反映済み。バージョン更新・リリースもv1.45.0として実施済み。

---

## 2026-09-22（Autoタブ「オートレイアウト」サブタブを追加、フキダシに「キャプション」種別を追加、v1.45.0）

前日実装した第1弾「ストーリー・脚本」サブタブに続き、第2弾として脚本からのコマ割り自動生成・既存レイアウトタブへの転送を実装した。計画・詳細は`PLAN_auto_tab.md`§12。ブランチ`feature/auto-tab`のまま。サブタブ名は既存の「レイアウト」タブと混同しないよう、ユーザー指定で**「オートレイアウト」**に確定した。

**自動配置アルゴリズム**（`static/js/auto-layout-core.js`、DOM非依存の新規モジュール）: 参考アプリ`manga_panel`の2機能（重要度按分=縦1列のみ、再帰分割=順序非保存の貪欲二分割）はどちらも単独では脚本の読み順を保持できないため、**読み順を保ったまま重要度で面積按分する再帰二分割**に独自に統合した。5スタイル（バランス/非対称/斜め継ぎ目/縦1列/横1列）、読み順（右から左/左から右）に対応。斜め継ぎ目スタイルはquadを4頂点で表現することで非矩形のコマにも対応する。

**UI**（`static/js/main/28b-auto-layout-tab.js`、モックアップ準拠）: 左ペイン（ページ設定/テンプレート/詳細設定のタブ切替）、中央ペイン（選択・分割ツール、ズーム、ページ送り付きのSVGプレビュー）、右ペイン（選択中コマのフキダシ追加・編集、画像の生成/ギャラリー選択）。「分割」ツールは選択中コマを二分割すると同時に対応する脚本コマも複製し、コマと脚本の1対1対応を常に保つ。テンプレートタブでは既存レイアウトタブの保存済みテンプレートをコマ数一致で流用できる。

**ページ設定の刷新**: サイズはページタブの「新規作成」ダイアログと同じプリセット方式（標準6種＋カスタム）にし、カスタムプリセットの保存先（localStorage）を共有することで両方から相互に使えるようにした。外側余白（上下左右、「均等」チェックボックスで連動編集可）を新設。既定コマ枠線幅を詳細設定タブに追加（従来ハードコードだった値を設定可能に）。「生成（全ページ）」ボタン（コマ割りを再計算するため確認ダイアログあり）はページ設定タブのみに配置し、既定フォント/既定サイズ/既定コマ枠線幅（生成には使わず転送時にのみ使う値）は詳細設定タブ専用の「保存」ボタン（確認ダイアログなし・再生成なし）に分離した。ユーザーから「生成ボタンでないとフォント設定が保存されないと誤解する／生成ボタンでコマ割りが変わると誤解する」との指摘を受けての対応。

**レイアウトタブへの転送**: プレビューで確定したコマ形状（手動分割済みのものを含む）からページレコードを組み立て、既存の半自動マンガ作成（`26-auto-comic-bridge.js`）と同じ関数群（`mapScriptPageToPanels`/`createBalloonAtPosition`/`applyBubbleTextToShape`/`insertImage`）でフキダシ・画像を流し込む。転送先ページは`_pageMgrGroups`/`_workMeta`経由でページ管理タブの「作品」としても登録する（登録しないとページ自体は編集できるが作品一覧に出ず見失いやすい）。

**実機で見つけて修正したバグ3件**:
1. 転送後ページの背景が透過していた（実際のテンプレートSVGにある`<rect fill="#ffffff">`背景矩形が生成SVGに無かった）。
2. 手動「分割」ツールで分割したコマが、転送後はコマ数は合っているのに配置が違っていた（転送処理が脚本panels[]から`layoutPanels()`を毎回再実行しており、プレビューでの手動分割結果を無視していた）。「既存quads→panels/svgContent変換のみ」と「新規生成」の関数を分離し、転送は必ずプレビューの確定quadsを直接使うよう修正。**プレビュー用の確定済みジオメトリを持っている場合、転送・保存等の最終処理では必ずそれを直接使い、元データから再計算してはならない**という教訓。
3. 外側余白の入力欄が1文字ラベル（上/下/左/右）に既存の長いラベル用固定幅クラスを流用していたため狭すぎて数値が読めず、狭い欄への誤入力で余白が`1011111010`という異常値になり、コマ配置領域がページ外へ押し出されて何も表示されなくなった。表示幅を修正し、`contentAreaFromMargins()`に余白合計がページサイズの90%を超えたら比例縮小する下限側の最終防御を追加、さらに読み込み時・保存時の両方でページ幅/高さ自体を上限に余白をクランプするようにした。**数値入力UIを狭くする際は、狭さゆえの誤入力でも壊れないか（境界値クランプ）を合わせて点検する**という教訓。

**フキダシに「キャプション」種別を追加**（関連作業）: オートレイアウトのナレーション用フキダシとして使うため、既存の角丸矩形（rect、角丸半径調整可・尻尾システム付き）とは別に、常に直角・尻尾なし専用の新種別「caption」を追加した。既存rectの「角丸半径0＋尻尾長さ0」設定で代用すると毎回の設定し直しが手間になるため、専用タイプとして独立させた。既存のフキダシ共通システム（`09b-balloon-shapes.js`）にcaption分岐を追加し、パラメータパネル（尻尾関連スライダー等）は表示させない設計。色・枠線変更・テキスト内包は既存フキダシと同様に使えるが、延長フキダシ機能は対象外（尻尾を持たないため）。レイヤーパネルに専用アイコン（▬）を追加。

**実機検証（Kapture）**: バランス/斜め継ぎ目/縦1列スタイル・右から左読み順・手動分割・フキダシ編集・画像への文字入り・転送・作品管理登録・上記3バグの修正・キャプションフキダシの挿入/色変更/テキスト内包/保存後の再描画、をいずれも確認。**未検証**: テンプレート対応付けタブ、画像生成/ギャラリーピッカー、非対称/横1列スタイル、左から右の読み順（LLM・画像生成の実行やスタイルの組み合わせが多く、今回は代表的な経路のみ確認）。

**未対応**: ヘルプ（ja/en/zh）・README（3言語）は第1弾分のみ反映済みで、第2弾（オートレイアウト・キャプションフキダシ）分は未反映（ユーザー確認後に反映する既定の運用フロー）。バージョン更新・リリースも未実施。

---

## 2026-09-21（Autoタブ「ストーリー・脚本」を追加、v1.45.0）
...
**未対応・次の予定**: 「レイアウト」サブタブ（脚本からのコマ割り自動生成、重要度によるコマサイズ按分、既存レイアウトタブ／スクリプトタブとの連携）は次セッション（2026-09-22）で着手済み、WFSの翻訳・VLMツール（レイアウトタブ作業後に検討）、I2Iは引き続き未着手。バージョン更新・リリースはv1.45.0として実施済み。ヘルプ（ja/en/zh）・README（3言語）に反映済み。
