...
**ヘッダーの高さ削減（同日、ユーザー要望）**: アプリタイトル「ComfyUI Comic Creator」を、タブ行の左端（同一行）へ移した（`static/css/style.css`の`header`をflex横並びに、`h1`の余白を0・16px・nowrapに、`.tabs`に`flex-wrap`、ヘッダーの上下paddingを縮小）。ヘッダーが約90pxから46px（1行）になり、全タブの作業領域の高さが増える。狭い画面ではタブが折り返す。
...
**サイズ拡張に「拡張ピクセル」方式を追加**: 従来の透明／白／黒に加え、元画像の縁のピクセルを引き伸ばして余白を埋める方式を追加した（`_edgeExtendFillCanvas()`）。単色塗りよりも境界の継ぎ目が目立ちにくく、後述のOutpaintの下地画像作成にも同じヘルパーを再利用する。余白色の選択ボタンは4択になったため、1列表示から2×2グリッド表示に変更した。

**Outpaint機能**: MaskツールのサブツールバーのInpaintボタンの右隣に「Outpaint」ボタンを追加（`_wfmAvailable`時のみ表示、Inpaintと同条件）。押すとツールプロパティペインに専用パネルが表示され、上下左右の余白（連動指定可）・余白の塗り方（拡張ピクセル／透明／白／黒）・Positive/Negativeプロンプト・Grow Mask By・Denoiseを設定してRunを押すだけで、次の3ステップを内部でまとめて実行する（`_runOutpaint()`）: ①現在の合成画像を指定した余白分拡張した下地画像を作成、②余白部分=白（生成対象）・元画像部分=黒（維持）のマスクを自動生成、③Inpaintと全く同じWorkflow Studio連携（`sendOutpaintToWorkflowStudio()` → WFS側の既存ブリッジ`_wfmReceiveInpaintRequest`をそのまま再利用）で送信。成功した場合のみキャンバスサイズを拡大・既存レイヤーをシフトし、結果を新規レイヤー「Outpaint Result」として追加する（失敗時はキャンバスに変更を加えない）。Inpaintとの違いは「事前にマスクレイヤーを手動で作る必要がない」点で、ユーザーからの要望（サイズ拡張とマスク作成の2段階操作を1回のRunにまとめたい）に応えた設計。

**設定タブに「Outpaint設定」を追加**: Inpaint設定と対称の構成（デフォルトワークフロー使用有無＋ファイル名）で、独立したlocalStorageキー（`ccc_outpaint_settings`）・独立したデフォルトファイル名（`cc_outpaint_default.json`）を持つ。Outpaintは画像の外側を生成するため、Image Editモデル等の専用ワークフローを使いたいケースを想定し、Inpaint設定とは別に切り替えられるようにした。

**64px単位への自動サイズ調整（実機検証で発覚した不具合の修正）**: 実機検証で、512×512の画像に下方向へ150px拡張してOutpaintを実行したところ、結果画像の一部が透過（チェッカーパターンが透けて見える）になる不具合が発生した。原因は、`VAEEncodeForInpaint`→`KSampler`→`VAEDecode`という拡散モデルの往復処理が、画像の幅/高さがVAEのダウンサンプリング係数（8）や、UNet内部の複数段ダウンサンプリングを考慮した安全な単位（64、ComfyUI標準の推奨値）の倍数でない場合、エンコード時に数ピクセル切り詰められた状態でデコードされて返ってくるため（512+150=662pxはどちらの倍数でもない）。返ってきた結果画像が拡張後キャンバスよりわずかに小さくなり、`_loadFromDataUrl()`のアスペクト比保持センタリングにより上下（または左右）に透過の隙間が生じていた。**修正**: `_outpaintAdjustedPad()`を新設し、Outpaint実行時・プレビュー表示時の両方で、実際に拡張する側の辺（ユーザーが0を指定した辺は変更しない）へ必要な分だけ余白を追加し、拡張後キャンバスサイズを64の倍数に切り上げる。プロパティパネルの「新しいサイズ」表示もこの調整後の値を表示し、「（生成モデルの制約により64px単位に自動調整されます）」と注記を追加した。

**検証**: 実機（Kapture）でWorkflow Studio連携の実際の生成ジョブまで実行して確認した。`comfyui-vrm-pose-editor`等と同様、Workflow Studio側のiframe内で使われているESモジュール（`comfyUI`シングルトン等）へ`<script type="module">`を動的挿入してdynamic importすることで、UIのクリック操作だけでは辿れないGenerate UI内部の解析結果（`comfyUI.currentAnalysis`）を直接読み書きし、ワークフロー読込のタイミング問題（後述）や解析結果の正しさを外部から検証できた。修正前（512+150=662px、8/64いずれの倍数でもない）で実際に透過ギャップが再現し、修正後（64の倍数である704pxに自動調整）で同条件のOutpaintを再実行したところ、キャンバス全域のアルファ値が255（完全不透明）になることを確認した。

**判明した別の注意点（コード修正なし）**: ユーザーの実機検証で「ワークフローを読み込んで即Runを押すとInpaint対応ノードが見つからないエラーになる」という現象が発生した。調査の結果、ワークフローのJSON自体（LoadImageのMASK出力がVAEEncodeForInpaintのmask入力に接続された標準的なOutpaint構成）や、Workflow Studio側の解析ロジック（`comfyui-workflow.js`の`analyzeWorkflow()`）自体には問題がなく、同じワークフローを読み込んで解析結果を直接確認すると正しく`mask_used: true`が検出された。ワークフロー読込時の`/object_info`取得が初回15〜20秒程度かかるため、**読込完了前にRunを押すと古い（デフォルトの）ワークフローの解析結果が使われてしまう**タイミング問題と判明。Comic Creator側の不具合ではないためコード修正は行っていないが、Outpaint/Inpaintのヘルプに今後同種の問い合わせがあれば「ワークフロー読込完了を待ってからRunする」旨を案内すること。

**How to apply**: [[comic-creator-workflow]]と同様、Workflow Studio側の内部状態（module-scopedなシングルトン等、`window`に公開されていない変数）を実機でデバッグする際は、対象のiframeの`contentDocument`に`<script type="module">`を動的追加してdynamic importすれば、既にロード済みのモジュールインスタンス（ESモジュールはURL単位でキャッシュされるため同一インスタンスが返る）へ`window`経由でアクセスできる。この手法は今後もWorkflow Studio連携機能のデバッグに再利用できる。また、生成AIモデルの往復処理を伴う新機能（Outpaint等）でキャンバス/画像サイズを扱う際は、VAEの8倍・UNetの内部ダウンサンプリングを考慮した64の倍数への丸めを常に検討すること（8の倍数では往復処理自体は通っても、UNet内部で解像度不整合が起きる可能性がある）。

ヘルプ（22-help-tab.js、日本語・英語・中国語）・README（3言語）に反映済み。このリリースはv1.44.0としてコミット・リリース実施。

---

## 2026-09-18（Imageタブ Select ツールに「サイズ拡張」を追加し、クロップUIをツールプロパティペインへ移動、v1.43.0）

Affinity Photo等の「キャンバスサイズ変更」機能を参考に、Imageタブに画像の四辺へ余白を追加してキャンバスサイズを拡張する機能を実装した（次に予定しているOutpaint機能の前段）。

**UI変更**: 選択ツールバーの「✂ クロップ」ボタンの右隣に「⛶ サイズ拡張」ボタンを追加。従来クロップのX/Y/幅/高さ入力・実行/キャンセルはツールバー（`ie-tool-options`）に直接表示していたが、今回これをツールプロパティペイン（`ie-props-pane`、Selectツール選択中は通常I2Iパネルを表示する場所）へ移動した（`_renderCropProps()`）。サイズ拡張のUIも同じ場所に表示する（`_renderExtendProps()`）。クロップ・拡張どちらのモードでもない場合は従来通りI2Iパネルを表示し、3つの表示を相互排他的に切り替える。

**サイズ拡張の実装**: 上/右/下/左を個別にピクセル単位で指定でき、「四辺を連動させる」チェックで1箇所の入力を四辺すべてに反映できる。余白の色は透明／白／黒から選択可能（`_applyExtend()`）。クロップ（`_applyCrop()`）が「キャンバスサイズを縮小し各レイヤーのx/yをクロップ原点分シフトするだけ」で実現しているのと対称的に、拡張は「キャンバスサイズを拡大し各レイヤーのx/yを追加した上・左の余白分シフトするだけ」で実現している（レイヤーはページ座標系に独立配置されているため、どちらも座標シフトのみで済む。詳細は`LayerManager.composite`参照）。透明以外の余白色を選んだ場合は、合成結果が透過背景のままだと書き出し画像に色が反映されないため、新キャンバス全体を塗りつぶす描画レイヤー（`Layer`）を`LayerManager.addLayer()`で作成し、配列末尾（最背面）へ移動してから追加する。

**レイアウト修正**: プロパティペインは幅160pxと狭く、`.ie-props-row`はデフォルトで`flex-direction: column`（ラベルの上に入力欄を1つ表示する縦積み）。当初、拡張パネルの「四辺を連動させる（均等に追加）」チェックボックスのラベルや「新しいサイズ: ◯◯ × ◯◯ px」の表示を1行のテキストとして詰め込んだところ、パネル幅を超えて横スクロールが発生した。ラベルには`white-space: normal`を明示して折り返し可能にし、「新しいサイズ」は`<br>`で2行に分割して解決。さらに、クロップ・拡張・既存のI2Iパネル（Positive/Negative Prompt・Workflow file入力）双方の数値/テキスト入力に共通する問題として、`width:100%`を指定した`<input>`/`<textarea>`が既定のcontent-boxのままだと、要素自身のpadding/borderが幅に加算されてパネル幅をわずかに超え、同じく横スクロールの原因になっていた。該当する全ての入力欄に`box-sizing: border-box`を明示して解決した。

**検証**: Kaptureで実機（`window._ccImageTab`のプロトタイプへ変更後のメソッドを直接代入してリロード無しに検証）を用い、四辺連動のチェック→数値同期→白選択→実行→1024×1024から1224×1224への拡張（背景に`Background Fill`レイヤーが最背面に追加）→クロップUIのプロパティペイン表示→Undoでの復元、を一通り確認済み。横スクロール解消も`ie-props-body`の`scrollWidth`と`clientWidth`が一致することを実機で確認。

**How to apply**: プロパティペイン（`ie-props-pane`、幅160px）に新しいUIを追加する際は、`.ie-props-row`が縦積みレイアウトである前提を踏まえ、複数の短いラベル+入力欄を1行に並べようとしない（狙って並べたい場合は`flex-direction:row`を明示指定した専用の行にする）。また同ペイン内の`<input>`/`<textarea>`に`width:100%`を指定する場合は必ず`box-sizing:border-box`も併記すること（このコードベースには`* { box-sizing: border-box }`のようなグローバルリセットが存在しないため）。

ヘルプ（22-help-tab.js、日本語・英語・中国語）・README（3言語）に反映済み。このリリースはv1.43.0としてコミット・リリース実施。
...
**1. キャンバス外オブジェクトの操作性**: HTML canvasの仕様上、要素の描画バッファを超えた部分は描画できずピクセル内容自体は表示できない。ユーザーとの合意で「ピクセルは見えなくてよいので、選択枠・ハンドルでの移動/回転/リサイズ操作ができれば十分」という条件に絞り、Selectツール専用の新しいcanvas要素`ie-canvas-select-overlay`を追加（`image-tab.js`／`templates/index.html`／`static/css/image-tab.css`）。キャンバス実サイズの四方に`margin = clamp(150, 500, max(w,h)*0.5)`だけ拡張したサイズで確保し、選択枠・角ハンドル・回転ハンドルがキャンバス外にはみ出しても表示・操作できるようにした。既存の`ie-canvas-overlay`（Crop/Blur/Shape/MaskVector/MaskShapeが使用）は変更せず、影響範囲をSelectツールに限定。`SelectTool.setCanvas(canvas, margin)`にmargin引数を追加し`ctx.translate(margin, margin)`で吸収することで、SelectTool内部のヒットテスト・座標計算はキャンバス座標系のまま変更不要にした。副次的に、キャンバス内側の透過部分に実際のチェッカーパターンを描画する`_drawTransparencyBg()`を追加し、ワークスペース背景（暗いグレー系）と区別できるようにしてキャンバス境界を視覚的に明示した。

**2. 3Dテキスト確定時のジャギー対策**: `image-tab/Text3DTool.js`の3Dプレビューは常に480×320(CSS)×devicePixelRatio固定の低解像度でレンダリングされ、`commit()`はそのままPNG化してレイヤーへ焼き込んでいたため、Imageタブ側でSelectToolを使って拡大配置すると粗さが目立っていた。当初「確定直前に固定3倍解像度でキャプチャ」で対応したが、ユーザーから「改善を感じる時と感じない時がある」「キャンバスが大きいとプレビューウィンドウが小さくて操作しづらい」という指摘を受け再修正。原因は、固定倍率だけではキャンバス座標系での実際の配置サイズ（ズーム状態により変動）を無視していたこと、およびプレビューのCSS表示サイズにズーム倍率をそのまま掛けていたため大きいキャンバスをズームアウトして編集しているとプレビューウィンドウ自体が縮んでいたこと。**レイアウトタブ（`main/25-text3d-bridge.js`）と比較**すると、レイアウトタブは配置先の「コマ」の実表示ピクセルサイズにプレビュー解像度をリアルタイム追従させているため常に必要十分な解像度になっており、ジャギーが出ない。Imageタブには「コマ」に相当する配置先の枠が無いため同じ方式はそのまま持ち込めず、代わりに`Text3DTool.js`の`open()`でオーバーレイの画面上CSS表示サイズを常に480×320固定にし（ズーム倍率を掛けない）、キャンバス座標系での配置サイズ(`displayW/displayH`)をズーム倍率の逆数から動的計算（キャンバス実サイズの60%を上限にクランプ）するよう変更。`commit()`のキャプチャ解像度も固定倍率ではなく、この配置サイズを基準に「1pxあたり最低2.5px（`TARGET_DENSITY`）」を確保する動的計算に変更した（`MAX_CAPTURE=4096`で上限クランプ）。

**検証**: このセッションでは全ファイルの構文チェック（`node --check`）のみ実施し、ComfyUIサーバーを起動してのブラウザ実機検証はユーザー自身が行った（「キャンバス外配置OKです」を確認、3Dテキストのジャギー改善は「確認できるときとできないときがある」状態でユーザーが許容し現状で確定とした）。

**既知の未解決点（次回申し送り）**: 3Dテキストレイヤーの再編集（`ie-select-text3d-edit-btn`）は既存レイヤーの`displayW/displayH`を`Text3DTool.open()`に渡しておらず、再編集の都度その時点のズーム状態から新しいサイズが再計算されてレイヤーへ上書きされる（今回のズーム連動化により再編集時にサイズが変わりやすくなった可能性がある）。

**How to apply**: 配置先に「コマ」のような固定枠がある機能（レイアウトタブ）ではプレビュー解像度をその枠の実表示サイズに追従させるのが最も確実。固定枠が無い機能（Imageタブの任意配置）では、キャンバス座標系での実際の配置サイズを基準にキャプチャ解像度を動的計算する方式が、固定倍率よりも環境・ズーム状態によるブレが少ない。

---

## 2026-09-09（Fillツールのグラデーション操作パネル、ペンタブレットでスライド操作ができない不具合を修正、v1.40.0）

ComfyUI-Workflow-Studio側でComic CreatorのImageタブの`FillTool.js`（バケツ塗りつぶし、Solid/Gradient対応）を移植した際、移植先で「Fillのグラデーション設定パネルにある色ストップランプ（Ramp）・方向ダイヤル（Direction）がペンタブレットでスライド操作できない」という実機フィードバックを受けた。原因を修正したところComic Creator本体（移植元）にも全く同じ不具合が存在することが判明したため、こちらにも同じ修正を横展開した。

**原因**: `image-tab.js`の`_setupFillGradientRamp()`（グラデーションランプのストップ選択・ドラッグ）、`_setupFillGradientDir()`（方向・強度ミニダイヤル）、および同一パターンのコードが複製されていたShapeツールの`_setupShapeGradRamp()`（塗りグラデーションランプ）の3箇所だけが、他のPointer Events化済みの描画系ツールと異なり`mousedown/mousemove/mouseup`のまま実装されていた。`touch-action`が未指定でブラウザのパン/スクロールジェスチャに奪われやすく、ドラッグ中の`mousemove`を`document`に張っていたためポインタキャプチャも無く、ペンの高速な動きを取りこぼしやすい状態だった。

**修正**: 3箇所とも`pointerdown/pointermove/pointerup/pointercancel`に変更し、対象canvasに`touch-action: none`を設定、`canvas.setPointerCapture(e.pointerId)`でドラッグ中はポインタをキャプチャ、ドラッグ中のリスナーも`document`ではなくcanvas自身に張るよう変更した。

**検証**: Kaptureで実機（ComfyUI_5上のComic Creator `/ccc`）を操作し、Fill/Shapeそれぞれのグラデーションランプでストップ選択（クリックで青ハイライト）が正常動作しコンソールエラーが無いことを確認。ペン特有のドラッグ挙動そのものは自動化ツールでは検証できないため未確認（本件はワークフロースタジオ側でのユーザー実機フィードバックが発端）。

**How to apply**: キャンバスベースの新規ドラッグ/描画UIは最初からPointer Events + `touch-action:none` + `setPointerCapture`で実装する。同一パターンのコードが複数箇所（今回はファイル間・同一ファイル内の両方）に複製されている場合、1箇所の不具合報告を機に全箇所を横展開で確認する。

このリリースはv1.40.0としてコミット・リリース実施。
...
**スコープ確認**: AskUserQuestionで2点を確認。(1) プレビュー表示だけでなく実テキストにも反映するか→実テキストにも反映（レイアウトタブ・Imageタブ両方）。(2) 既存の文字寄せドロップダウン（Imageタブ専用として実装済み、レイアウトタブ未適用）をボタン形式に変更するか→ボタン形式に変更し、レイアウト/Imageタブで統一する。

**実装**:
- SVGテキスト（レイアウトタブ、`09e-text-tool.js`）: `_setTextElVertical` を拡張し、`dataset.lineHeightMult`/`dataset.textAlign`/`dataset.textValign`（既定値は旧来の固定値と同一＝1.2em・左寄せ・上基準で後方互換）から複数行tspanのdy/dx（すべて'em'相対単位。フォントサイズ変更時に行間比率が自動追従するようリサイズハンドルの挙動を壊さないため）と`text-anchor`を再構築するよう変更。`_fontMgrApplyStyleAttrsToTextEl`（スタイル適用の共通入口）にdataset設定＋tspan未生成時の改行分割フォールバックを追加し、`_fontMgrExtractStyleFromTextEl`（モーダル再オープン時の初期値復元）にも新フィールドの読み取りを追加。
- Canvasテキスト（Imageタブ、`image-tab.js`）: `_measureTextBox`/`_rerenderTextLayer` の行間を`p.lineHeightMult`可変化。`_applyTextPropsToLayer`のレイヤー高さ変化時の基準点をvalign（top/center/bottom）に応じて可変化（従来は常に中心固定）。
- UI: フォント管理タブ（`19-font-manager.js`、`templates/index.html`）と`text-style-modal.js`の両方で、プレビュー文字欄を`<textarea>`化（既定値2行）、サイズ下に行間スライダー追加、文字寄せセレクトをフキダシと同じボタン形式＋上下位置ボタンに置き換え。i18n（日英中3言語）追加。

**副次的に発見・修正した不具合（Imageタブの行間クリッピング）**: 実機検証中、ユーザーからImageタブで行間を0.8（1.2未満）にすると文字下部が切れるとの報告。原因は`_measureTextBox`のCanvas高さ計算が「行数×行間」のみで、行間を1.2未満にすると1行分のグリフ自体の実高さ（降下部分等）より確保領域が小さくなっていたため。最終行の実高さ分は常に`fontSize×1.2`（従来固定値）で確保し、行間はそれ以外の行の間隔にのみ適用するよう修正。

**検証**: Kaptureで実機検証（本セッション途中でKaptureとの接続が切断されたため、Imageタブのクリッピング修正の再確認は`node`のvm.SourceTextModuleによるESM構文検証のみ）。レイアウトタブでは、実テキスト作成→行間2.0/文字寄せ右/上下位置中央でスタイル適用→tspanのdy/x/text-anchor・getBBoxによるベースライン位置を数値検証（想定通り、上下位置=centerで2行のベースライン中点が元のy座標と一致することを確認）、縦書き切替でもクラッシュしないことを確認。Imageタブでは、行間0.8適用前後のスクリーンショットで文字欠けが解消されたことを確認。i18n・ヘルプ・README更新済み。

---

## 2026-08-26（ページ管理タブのテンプレートに「ページとして挿入」ボタンを追加、v1.36.0）

ユーザー依頼: ページタブの「テンプレート」サブタブで選択したテンプレートを、開いている作品へ新規ページとして挿入できるようにしたい（レイアウトタブのアセットパネル「テンプレート」タブの機能と同じ）。

**実装**: グループ絞り込みドロップダウンの右隣にボタンを追加し、既存の`insertTemplatePageToWork()`（アセットパネル版と共通の関数、`11a-work-manager.js`）をそのまま呼び出すだけで実現。`06b-template-manager.js`から`11a-work-manager.js`への新規import（循環importになるが、参照は関数内部に閉じているため既存のG1/G2と同じ判断基準で安全）。選択・削除・リネーム時のボタン有効/無効化は既存の削除・名前変更ボタンと同様に同期。

**検証**: Kaptureで実機検証済み。テンプレート「2」を選択→ボタン押下→開いていた作品にページ3/3として追加されることを確認。

---

## 2026-08-26（3DポーズにカメラFOV・Nearスライダーを追加、v1.36.0）

ユーザー依頼: vrm-pose-editor側のアップデートでカメラのFOV・Nearに対応したので、レイアウトタブの3Dポーズにも反映したい（「コマに配置」ボタン左隣にスライダーを設置）。

**実装**: `pose_editor_core.js`（`comfyui-vrm-pose-editor`、共有コア）側は既にFOV/Near設定用の`setFov`/`getFov`/`setNear`/`getNear`をpublic APIとして持っていたため、SPA側（`templates/index.html`、`23-pose3d-bridge.js`）はUIスライダー追加とイベント配線のみで対応（FOV: 10〜120°・既定45、Near: 0.01〜5・既定0.1、ComfyUIノード側UIと同じ値域）。i18n（日英中3言語）追加。

**検証**: Kaptureで実機検証済み。VRMモデル読込後、FOVスライダーを45→110→45で見た目のズーム変化を確認、NearスライダーをNear=3にしてカメラより近いオブジェクトが正しくクリップされることを確認。
