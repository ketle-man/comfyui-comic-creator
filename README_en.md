...
- **Groups and layer panel** — Group objects, manage stacking order, toggle visibility, lock, and **delete with the Delete / Backspace key**. **Alt+click** the visibility icon to toggle every object of the same type (balloon, image, text, or drawn shape) across the whole page at once — all panels, the overlay, and the draft layer, including objects nested inside groups
- **Asset registration** — The 📦 button on a group row registers the group as an SVG asset (choose a target group or create a new folder → enter a filename → register; it is reflected in the asset panel immediately). The **📦+** button at the top of the layer panel (to the left of the ↑↓ buttons) lets you register an ungrouped individual item (image, balloon, text, or drawn shape) the same way — check its box, **exactly one** at a time (checking two or more shows a message). When the single selected item is an image, it is saved directly as the image itself instead of being wrapped in an SVG group, so inserting it into a panel later doesn't create an unnecessary group. The checkbox is cleared automatically once registration finishes
...
- **Generate integration** (formerly I2I integration) — The "Generate" button opens a modal where you switch between three targets — a selected image, the whole page, or a single panel (picked via the prompt tabs) — and run I2I/T2I generation via Workflow Studio on the spot. Prompts are split into an "Overall" tab plus one "Panel N" tab per panel on the current page, each with its own independent Positive/Negative. A **batch** checkbox appears only when the target is the whole page: on, it batch-generates every panel on the current page (combining the "Overall" tab prompt with each panel tab's prompt) and replaces each panel's image in turn; off, it keeps the previous behavior of compositing the whole page into one flat image and always inserting the result into the overlay at full page size (for rough previews). A **T2I** checkbox next to the Run button switches to generating from text only, without an input image (the Denoise field is hidden while it's on). Default workflows to auto-load can be configured independently for I2I and T2I in the modal's collapsible "Workflow Settings" section (the I2I setting is shared with the I2I panel of the Image tab's Select tool, via [ComfyUI-Workflow-Studio](#optional-dependencies)). A **Fetch from Auto** button fills the story of the work currently open in the Auto tab into the "Overall" tab, and the background/acting direction of each panel of the selected script page into the corresponding "Panel N" tab, as Positive prompts (overwriting existing input; panels are matched by order only; independent of transferring from Auto's Auto Layout)
- **PixiJS FX** — Apply particle/filter effects to the selected image from the "Image" sub-tab. Supports stacking and reordering multiple filters, with combinations saveable/loadable/deletable as presets (via [comfyUI-particle-pixijs](#optional-dependencies))
...
- **Outpaint (Workflow Studio integration)** — Used via the **Outpaint** button next to Inpaint in the Mask tool (no need to create a mask layer beforehand). Set the Top/Right/Bottom/Left padding (can be linked), the margin fill (Edge Pixels / transparent / white / black), Positive/Negative prompts, Grow Mask By, and Denoise, then click Run to do it all in one step: (1) composite a base image with the canvas extended by the padding, (2) auto-generate a mask (white = margin to generate, black = original area to keep), and (3) send both to Workflow Studio — on success the canvas is resized and the result added as a new layer. Since diffusion models' VAE encode/decode round-trip crops a few pixels off when the image size isn't a multiple of 64px (which would leave a transparent gap in the result), the padding on the side(s) being extended is automatically rounded up to a multiple of 64px before sending. Its default workflow can be set independently from Inpaint's, via "Outpaint Settings" in the Settings tab
- **PixiJS FX** — Apply particle/filter effects to the active layer from a toolbar button. Supports stacking and reordering multiple filters, with combinations saveable/loadable/deletable as presets (via [comfyUI-particle-pixijs](#optional-dependencies))
...
<p>
  <img src="docs/11_pixifx.png" width="260" alt="PixiJS FX multi-filter stacking and reordering">
