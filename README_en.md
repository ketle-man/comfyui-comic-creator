...
- **Semi-automatic manga creation** — the "Map this page to the layout" button maps each panel of a template-applied Layout-tab page to the script's panels in panel-number order. The "Auto-generate balloons" button then automatically creates dialogue-filled balloons (fixed rounded-rectangle shape) in each mapped panel based on that mapping (adjustable afterward with the existing manual editing tools). The "L" button and "Batch-generate images" button add Workflow Studio-based prompt drafting (Ollama/LM Studio) and batch image generation sized to each panel's aspect ratio
