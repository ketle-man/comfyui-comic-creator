...
- **Paint tool** — A raster-brush painting feature, separate from vector shape drawing. Clicking "Add Paint" creates a transparent image sized exactly to the selected panel/overlay/draft layer, selects it automatically, and turns Draw ON so you can start brushing right away. Supports brush color, opacity, and size (slider), plus x5 (5x brush size) and eraser toggles whose ON/OFF state is shown by button color. Paint objects appear in the layer panel with a 🖌 icon and the name "Paint" to distinguish them from regular images. Useful for rough sketches to edit in the Image tab, or for creating I2I reference images
...
- **I2I integration** — Send either a selected image (I2I) or a PNG render of the current page (PI2I) to Workflow Studio's Generate UI and bring the result back. A default workflow to auto-load can be configured in the Settings tab (via [ComfyUI-Workflow-Studio](#optional-dependencies))
